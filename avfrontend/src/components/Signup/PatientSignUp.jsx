  ...
  const handleChange = (e) => {
    ...
    if (name === "password") {
      ...
      if (!passwordRegex.test(value)) {
        setPasswordError(
          "Password must contain at least one lowercase letter, one uppercase letter, one number, one symbol, and have a minimum length of 8 characters"
        );
      } else {
        setPasswordError("");
      }
    ...
    if (passwordError) {
      ...
      try {
        const response = await axios.post(
          "https://localhost:8090/patient/createPatient",
          formData
        );
