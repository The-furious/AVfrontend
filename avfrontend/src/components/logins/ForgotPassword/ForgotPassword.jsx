...
function ForgotPassword({ onPasswordReset }) {
  ...
  const handleSendOtp = async () => {
    try {
      const response = await axios.post(
        "https://localhost:8090/auth/createOTP",
        {
  ...
  const handleValidateOtp = async () => {
    try {
      const response = await axios.post(
        "https://localhost:8090/auth/verifyOTP",
        {
          username: email, // Use the email as the username
          otp: otp,
        }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const response = await axios.post(
        "https://localhost:8090/auth/changePassword",
        {
          username: email, // Use the email as the username
      ...
      {passwordResetSuccess && (
        <div className="popup">
          <div className="popup-content">
            <span className="close-btn" onClick={handleCloseSuccessMessage}>
              &times;
            </span>
